...
def deleter_thread(exit_event: threading.Event):
  while not exit_event.is_set():
    ...
    for n, delete_dir in enumerate(recording_dirs):
      delete_path = os.path.join(Paths.log_root(), delete_dir)
      segment_over_limit = len(recording_dirs) - n > MAX_RECORDING_SEGMENTS
      if segment_mtime(delete_path) >= max_recording_age and not segment_over_limit:
        continue
      if any(name.endswith(".lock") for name in os.listdir(delete_path)):
        continue

      try:
        reason = f"older than {MAX_RECORDING_AGE_DAYS} days" if segment_mtime(delete_path) < max_recording_age else f"recordings over {MAX_RECORDING_SEGMENTS} segments"
